...
public function get_welcome_doctor_cat($category_id){
  echo $cat_id = $category_id;
    ...
    public function find_trending_blogs_new($blog_user){
        ...
        $data = $this->db_blog->query("SELECT * FROM wp_posts where doctor_id = '$d_id' and post_status = 'publish' ");
		$data_doctors = $this->getById($blog_user,'doctors');	
        ...
        if($data->num_rows > 0)
        {
            ...
            while($res=$data->fetch_assoc())
            {
                ...
                if($count<6){
                $str.='<div class="col-sm-12" style="margin-top:10px;width:100%; border:1px solid #ddd">
			        <div class="row">
        			<div>
        				<div>
        					<a target="blank" style="font-size: 1.1em;color:#333;text-decoration:none;" href="';
        					
        					
							$id = $res['ID'];				
							$post_name = $this->db_blog->query("SELECT * FROM `wp_posts` where ID = '$id'");
							$post_name = $post_name->fetch_assoc();
							$img_array = $this->get_wordpress_data_by_id($id+1, "wp_posts", 'guid');
							$post_titles = $post_name['post_title'];
							$length = strlen($post_titles);
							if($length > 20){
								$post_titles = substr($post_titles,0,20).'...';
							}else{
								$post_titles = $post_titles;
							}
							
							$str.=''.base_url.'blog/'.str_replace(" ","-",$post_titles).'/">';
							$str.='</br>';
							if($img_array[0] != ''){
								$blog_img = $img_array[0];
							}else{
								$blog_img = base_url."blog/wp-content/uploads/bloger.png";
							}
							
							$str.='<div class="col-md-2"><img src="'.$blog_img.'" alt="blog-image" style="width:25px; height:25px;" ></div>';
							//$str.='</br>';
							$str.= "<div class='col-md-9' style='font-size:13px;text-align:left'>".$post_titles."</div>"; 
							$str.='</br>';
							...
							$str.='</a><br>
        					<p style="font-size: .7em;color: #828282;margin-top: 0.2em;margin-left:5px;" class="text-justify"> ';
							if($post_name['post_status'] == 'publish'){$blog_status = "Publish by: ";}else{$blog_status = " ";}
							$str.= $blog_status.$data_doctors['be_name'].$author_name['display_name']."<br>";
							$str.= "<small align='left'>".date('d-m-Y',strtotime($post_name['post_date']))."</small>";
        					$str.='</p>
        				</div>
        			</div>
        		</div>
...
{
	$id=$_SESSION['login_id'];
	$res=$this->db->query("SELECT *,app_date as date_app FROM `request_for_call` WHERE doctor_id = '$id' and status = 'Pending' ORDER BY app_date desc ");
...
public function getdistrict($query)
{
    ...
    public function getdiscondition($query)
{
...
{	$id=$_SESSION['login_id'];
	$res=$this->db->query("select sms.date, sms.message, sms.status, patient.name, patient.phone_no from sms left JOIN patient ON sms.patient_id = patient.id where  sms.doctor_id='$id' order by sms.id desc ");
...
public function add_doctor_expneses($docclinic,$docexpanse,$docmode,$docvendor,$docamount,$docdate,$docid)
{
	$doc_id=$_SESSION['login_id'];
	$date1=date('Y-m-d');
	
	$res=$this->db->query("INSERT INTO doctor_expenses(clinic,expense, vendor, date, mode_of_payment, amount, doctor_id, stored_date) VALUES ('$docclinic','$docexpanse','$docvendor','$docdate','$docmode','$docamount','$doc_id','$date1')   ");
	if(@$res)
	{
...
public function edit_doctor_expneses($docclinic,$docexpanse,$docmode,$docvendor,$docamount,$docdate,$docid,$id)
{
    $date1=date('Y-m-d');
    
    $res=$this->db->query("UPDATE doctor_expenses SET clinic='$docclinic',expense='$docexpanse',vendor='$docvendor',date='$docdate',mode_of_payment='$docmode',amount='$docamount',doctor_id='$docid',stored_date='$date1' WHERE id='$id'");
    if(@$res)
    {
        $active_name="expenses";
...
public function delete_doctor_expneses($id)
{
    
    $date1=date('Y-m-d');
    
    $this->db->query("DELETE FROM doctor_expenses WHERE id='$id'");
        return 1;
}
public function display_doctor_expenses($docid)
{
	


	$res=$this->db->query("SELECT * FROM doctor_expenses WHERE doctor_id='$docid'");	
...
{
	if($patient_name == ''){$patient_name = 'off';}
	if($clinic == ''){$clinic = 'off';}
	if($category == ''){$category = 'off';}
	if($contact == ''){$contact = 'off';}
	...
	else
	{
		...
		$res1=$this->db->query("update setting set patient='off',clinic='off',category='off',contact='off' where doctor_id='$doc_id' and temp_name='confirm_msg' ");
	}
	if(@$res1)
	{
		return true;
...
public function cancel_msg($patient_name,$clinic,$category,$contact,$cancel)
{
	...
	{
		$res1=$this->db->query("update setting set patient='off',clinic='off',category='off',contact='off' where doctor_id='$doc_id' and temp_name='cancel_msg' ");
...
public function reminder_msg($patient_name,$clinic,$category,$contact,$reminder)
{
	...
	{
		$res1=$this->db->query("update setting set patient='off',clinic='off',category='off',contact='off' where doctor_id='$doc_id' and temp_name='reminder_msg' ");
...
public function add_templete($msg,$tmp,$bir)
{
 	...
 	if($bir=="on")
	{
		...
		if($res->num_rows>0)
		{
			$data=$res->fetch_assoc();
			$id=$data['id'];
			$res1=$this->db->query("update msg_tamplete set templete='$tmp',msg='$msg',status='on' where id='$id' ");
		}
		else{
				$res1=$this->db->query("insert msg_tamplete(templete,msg,doctor_id,status) values('$tmp','$msg','$doc_id','on')");
		}
    }
    else
    {
    	$res1=$this->db->query("update msg_tamplete set status='off' where doctor_id='$doc_id' and templete='$tmp' ");
...
public function add_email_setting($confirm,$cancel,$reminder,$follow,$birthday,$follow_period)
{
	$doc_id=$_SESSION['login_id'];
	/*if($follow=="on")
	{*/
		if($confirm == ''){$confirm = 'off';}
		if($cancel == ''){$cancel = 'off';}
		if($reminder == ''){$reminder = 'off';}
		if($follow == ''){$follow = 'off';}
		if($birthday == ''){$birthday = 'off';}
		...
		if($res->num_rows>0)
		{
			$data=$res->fetch_assoc();
			$id=$data['id'];
			$res1=$this->db->query("update email_setting set confirm='$confirm',cancel='$cancel',reminder='$reminder',follow='$follow', birthday='$birthday',follow_period='$follow_period' where doctor_id='$doc_id' ");
		}
		else{
				$res1=$this->db->query("insert email_setting(confirm,cancel,reminder,follow,birthday,doctor_id,follow_period) values('$confirm','$cancel','$reminder','$follow','$birthday','$doc_id','$follow_period')");
		}
...
$str2 = '<table class="head-wrap"  style="width: 650px; margin: 0 auto; background: linear-gradient(to right, #2D9CDB, #2F5894);">
    <tr>
        <td></td>
        <td class="header container">
            ...
            <table>
                <tr>
                    <td><a href="'.base_url.'" style="margin-left:10px;display:block" target="_blank"><img src="https://docconsult.in/image/logo.png" width="200px" height="70px"></a></td>
                </tr>
            </table>
            </div>
        ...
        <td class="header container">
            ...
            <table>
                    <tr>
                        ...
                        <br>
                            <h3>Hello, Admin</h3>
                            <p class="lead">An appointment has been Cancelled by '.$doctor['be_name'].$doctor['name'].'. This mail is <a style="color:#02a6d8;text-decoration:none" href="'.base_url.'" target="_blank" > Generated by Docconsult.</a> Here are the details:</p>
                        </td>
                    </tr>
                </table>
            </div>          
...
<table class="head-wrap" style="width: 650px;    background: #f5f5fa;    margin: 0 auto; ">
    <tr>
        ...
        <td class="header container">
            ...
            <div style="max-width: 630px;    margin: 0 auto;    display: block;">
                <table>
  <tr>
   <td style="border-bottom: 1px dashed #777777; width: 100%;"><span style="text-decoration: none; color: #666;     padding: 3px 0 5px 0;   min-width: 180px; font-weight: bold;   text-align: left;  float:left;">Appoinment taken for :
</span><span style="text-decoration: none; color: #666;  padding: 10px 0;     text-align: center;     ">'.$patient_name.'</span>
</td>
  ...
  <h3 style="margin:5px 0 5px 0;">Doctor Details</h3>
   ...
   <tr>
   <td style="border-bottom: 1px dashed #777777; width: 100%;"> <span style="text-decoration: none; color: #666;     padding: 3px 0 5px 0;  min-width: 180px;font-weight: bold;   text-align: left;   float:left;">Doctor Name :
</span><span style="text-decoration: none; color: #666;  padding: 10px 0;     text-align: center;      ">'.$doctor_name.'</span></td>
    
  </tr>
  <tr>
   <td style="border-bottom: 1px dashed #777777; width: 100%;"><span style="text-decoration: none; color: #666;     padding: 3px 0 5px 0;min-width: 180px; font-weight: bold;   text-align: left;  float:left;">Doctor Email:

</span><span style="text-decoration: none; color: #666;  padding: 10px 0;     text-align: center;    ">'.$doctor['email'].'</span></td>
    
  </tr>
   <tr>
   <td style="border-bottom: 1px dashed #777777; width: 100%;"> <span style="text-decoration: none; color: #666;     padding: 3px 0 5px 0;   min-width: 180px; font-weight: bold;  text-align: left;   float:left;">Doctor Mobile No.

</span><span style="text-decoration: none; color: #666;  padding: 10px 0;      text-align: center;     ">'.$doctor['phone_no'].'</span></td>
...
</span><span style="text-decoration: none; color: #666;  padding: 10px 0;      text-align: center;     ">'.$clinic['name'].'</span></td>
...
</span><span style="text-decoration: none; color: #666;  padding: 10px 0;      text-align: center;      ">'.$clinic['address'].', '.$clinic['area'].', '.$clinic['city'].'</span></td>
    
  </tr>
 
  <tr>
  <td>
  <h3 style="margin:5px 0 5px 0;">Patient Detail</h3>
  </td>
  </tr>
  
  <tr>
   <td style="border-bottom: 1px dashed #777777; width: 100%;"><span style="text-decoration: none; color: #666;    padding: 3px 0 5px 0;min-width: 180px; font-weight: bold; margin-right: 10px;  text-align: left;  float:left;">Pateint Name:

</span><span style="text-decoration: none; color: #666;  padding: 10px 0;     margin-right: 10px;  text-align: center;   ">'.$data['patient_name'].' </span></td>
    
  </tr>
   <tr>
   <td style="border-bottom: 1px dashed #777777; width: 100%;"><span style="text-decoration: none; color: #666;     padding: 3px 0 5px 0;  min-width: 180px;  font-weight: bold; margin-right: 10px;  text-align: left;   float:left;">Patient Mobile no :

</span><span style="text-decoration: none; color: #666;  padding: 10px 0;     margin-right: 10px;  text-align: center;     ">'.$phone.'</span></td>
    
  </tr>
  <tr>
   <td style="border-bottom: 1px dashed #777777; width: 100%;"><span style="text-decoration: none; color: #666;     padding: 3px 0 5px 0;  min-width: 180px; font-weight: bold; margin-right: 10px;  text-align: left;   float:left;">Pateint Email


</span><span style="text-decoration: none; color: #666;  padding: 10px 0;     margin-right: 10px;  text-align: center;      ">'.$data['patient_email'].'</span></td>
...
</span><span style="text-decoration: none; color: #666;  padding: 10px 0;      text-align: center;     ">'.$time.'</span></td>
    
  </tr>
  <tr>
   <td style="border-bottom: 1px dashed #777777; width: 100%;"><span style="text-decoration: none; color: #666;     padding: 3px 0 5px 0;   min-width: 180px; font-weight: bold;   text-align: left;   float:left;">Remark by Doctor

</span><span style="text-decoration: none; color: #666;  padding: 10px 0;      text-align: center;     ">'.$data['remark'].'</span></td>
        ...
        <td class="header container">
            ...
            <table>
                <tr>
                    ...
                    <p style="  font-size: 12px;     margin: 2px 10px;  color:#fff;">Contact NO : <strong> 0141-2785199</strong></p>
                    <p style="  font-size: 12px;      margin: 2px 10px;  color:#fff;">Email: <strong><a href="emailto:info@docconsult.in" style="color:#fff;">info@docconsult.in</a></strong></p>
                    <p style="   font-size: 12px;     margin: 2px 10px;  color:#fff;">Copyright © 2017 DocConsult | All Right Reserved 2016</p> </td>
...
$str.= '
			  ...
			  <tr>
						<td></td>
						<td class="header container"><div style="max-width: 630px;    margin: 0 auto;    display: block;">
							<table>
							<tr>
								<td><br>
								<h3>Dear , '.$patient_name.',</h3>
								<p class="lead"> <b>your Appointment Reschedule. by '.$doctor['be_name'].' '.$doctor['name'].' </b></p></td>
							  </tr>
						  </table>
						  </div></td>
						<td></td>
			  ...
			  <tr>
						<td></td>
						<td class="header container"><!-- content -->
				  <div style="max-width: 630px;    margin: 0 auto;    display: block;">
							<table>
					  <tr>
								<td style="border-bottom: 1px dashed #777777; width: 100%;"><span style="text-decoration: none; color: #666;     padding: 3px 0 5px 0;   min-width: 180px; font-weight: bold;   text-align: left;  float:left;">Appoinment With Doctor : </span><span style="text-decoration: none; color: #666;  padding: 10px 0;     text-align: center;     ">'.$doctor['be_name'].' '.$doctor['name'].'</span> </td>
							  </tr>
					  <tr>
								<td><h3 style="margin:5px 0 5px 0;">Doctor Details</h3></td>
							  </tr>
					  <tr>
								<td style="border-bottom: 1px dashed #777777; width: 100%;"><span style="text-decoration: none; color: #666;     padding: 3px 0 5px 0;  min-width: 180px;font-weight: bold;   text-align: left;   float:left;">Doctor Name : </span><span style="text-decoration: none; color: #666;  padding: 10px 0;     text-align: center;      ">'.$doctor_name.'</span></td>
							  </tr>
					  <tr>
								<td style="border-bottom: 1px dashed #777777; width: 100%;"><span style="text-decoration: none; color: #666;     padding: 3px 0 5px 0;min-width: 180px; font-weight: bold;   text-align: left;  float:left;">Appointment Date : </span><span style="text-decoration: none; color: #666;  padding: 10px 0;     text-align: center;    ">'.$date.'</span></td>
							  </tr>
					  <tr>
								<td style="border-bottom: 1px dashed #777777; width: 100%;"><span style="text-decoration: none; color: #666;     padding: 3px 0 5px 0;   min-width: 180px; font-weight: bold;  text-align: left;   float:left;">Appointment Time : </span><span style="text-decoration: none; color: #666;  padding: 10px 0;      text-align: center;     ">'.$time.'</span></td>
							  </tr>
					  <tr>
								<td style="border-bottom: 1px dashed #777777; width: 100%;"><span style="text-decoration: none; color: #666;     padding: 3px 0 5px 0;   min-width: 180px; font-weight: bold;   text-align: left;   float:left;">Clinic Name : </span><span style="text-decoration: none; color: #666;  padding: 10px 0;      text-align: center;      ">'.$clinic['name'].'</span></td>
							  </tr>
					  <tr>
								<td style="border-bottom: 1px dashed #777777; width: 100%;"><span style="text-decoration: none; color: #666;     padding: 3px 0 5px 0;   min-width: 180px; font-weight: bold;   text-align: left;   float:left;">Clinic Address : </span><span style="text-decoration: none; color: #666;  padding: 10px 0;      text-align: center;      ">'.$clinic['address'].', '.$clinic['area'].', '.$clinic['city'].'</span></td>
							  </tr>
					  <tr>
								<td><h3 style="margin:5px 0 5px 0;">Patient Detail</h3></td>
							  </tr>
					  <tr>
								<td style="border-bottom: 1px dashed #777777; width: 100%;"><span style="text-decoration: none; color: #666;    padding: 3px 0 5px 0;min-width: 180px; font-weight: bold; margin-right: 10px;  text-align: left;  float:left;">Pateint Name: </span><span style="text-decoration: none; color: #666;  padding: 10px 0;     margin-right: 10px;  text-align: center;   ">'.$patient_name.' </span></td>
							  </tr>
					  <tr>
								<td style="border-bottom: 1px dashed #777777; width: 100%;"><span style="text-decoration: none; color: #666;     padding: 3px 0 5px 0;  min-width: 180px;  font-weight: bold; margin-right: 10px;  text-align: left;   float:left;">Remark : </span><span style="text-decoration: none; color: #666;  padding: 10px 0;     margin-right: 10px;  text-align: center;     ">'.$data['remark'].'</span></td>
							  </tr>
					  <tr>
								<td style="border-bottom: 1px dashed #777777; width: 100%;"><span style="text-decoration: none; color: #666;     padding: 3px 0 5px 0;  min-width: 180px; font-weight: bold; margin-right: 10px;  text-align: left;   float:left;">Pateint Email </span><span style="text-decoration: none; color: #666;  padding: 10px 0;     margin-right: 10px;  text-align: center;      ">'.$data['patient_email'].'</span></td>
							  </tr>
					  <tr>
								<td style="border-bottom: 1px dashed #777777; width: 100%;"><span style="text-decoration: none; color: #666;     padding: 3px 0 5px 0;min-width: 180px; font-weight: bold;   text-align: left;  float:left;">Appointment Date : </span><span style="text-decoration: none; color: #666;  padding: 10px 0;     text-align: center;    ">'.$date.'</span></td>
							  </tr>
					  ...
					  <tr>
								<td style="border-bottom: 1px dashed #777777; width: 100%;"><span style="text-decoration: none; color: #666;     padding: 3px 0 5px 0;   min-width: 180px; font-weight: bold;   text-align: left;   float:left;">Remark by Doctor </span><span style="text-decoration: none; color: #666;  padding: 10px 0;      text-align: center;     ">'.$data['remark'].'</span></td>
							  </tr>
					  <tr>
								<td></td>
								<td></td>
				  ...
				  <div  style="max-width: 630px;     margin: 0 auto;    display: block;">
							<table>
					  <tr>
								<td><h4>Thank You For Using Docconsult.</h4>
						  <p class="lead"> If you find this is useful for you please give star rating and feedback. <br>
									Email us if you have any questions or feedback <br>
									Please Visit on <a href="'.base_url.'"></a></p>
						  <p class="lead">Regards,<br>
									Docconsult Team</p></td>
							  </tr>
					</table>
						  </div></td>
						<td></td>
...
$str.= $this->email_footer();			
  			...
  			<tr>
                ...
                <td class="header container"><div style="max-width: 630px;    margin: 0 auto;    display: block;">
                    ...
                    <tr>
                        ...
                        <p class="lead">An appointment has been Reschedule by you. This mail is <a style="color:#02a6d8;text-decoration:none" href="'.base_url.'" target="_blank" > Generated by Docconsult.</a> Here are the details:</p></td>
                      </tr>
                  </table>
                  </div></td>
                <td></td>
            ...
            <table class="head-wrap" style="width: 650px;    background: #f5f5fa;    margin: 0 auto; ">
  			<tr>
                ...
                <td class="header container"><!-- content -->
                    ...
                    <table>
          		<tr>
                        <td style="border-bottom: 1px dashed #777777; width: 100%;"><span style="text-decoration: none; color: #666;     padding: 3px 0 5px 0;   min-width: 180px; font-weight: bold;   text-align: left;  float:left;">Appoinment taken for : </span><span style="text-decoration: none; color: #666;  padding: 10px 0;     text-align: center;     ">'.$patient_name.'</span> </td>
                      </tr>
          		<tr>
                        <td style="border-bottom: 1px dashed #777777; width: 100%;"><span style="text-decoration: none; color: #666;     padding: 3px 0 5px 0;  min-width: 180px;  font-weight: bold; margin-right: 10px;  text-align: left;   float:left;">Patient Mobile no : </span><span style="text-decoration: none; color: #666;  padding: 10px 0;     margin-right: 10px;  text-align: center;     ">'.$phone.'</span></td>
                      </tr>
          		<tr>
                        <td style="border-bottom: 1px dashed #777777; width: 100%;"><span style="text-decoration: none; color: #666;     padding: 3px 0 5px 0;  min-width: 180px; font-weight: bold; margin-right: 10px;  text-align: left;   float:left;">Pateint Email </span><span style="text-decoration: none; color: #666;  padding: 10px 0;     margin-right: 10px;  text-align: center;      ">'.$data['patient_email'].'</span></td>
                      </tr>
         		...
         		<tr>
                        <td style="border-bottom: 1px dashed #777777; width: 100%;"><span style="text-decoration: none; color: #666;     padding: 3px 0 5px 0;min-width: 180px; font-weight: bold;   text-align: left;  float:left;">Doctor Email: </span><span style="text-decoration: none; color: #666;  padding: 10px 0;     text-align: center;    ">'.$doctor['email'].'</span></td>
                      </tr>
          		<tr>
                        <td style="border-bottom: 1px dashed #777777; width: 100%;"><span style="text-decoration: none; color: #666;     padding: 3px 0 5px 0;min-width: 180px; font-weight: bold;   text-align: left;  float:left;">Appointment Date : </span><span style="text-decoration: none; color: #666;  padding: 10px 0;     text-align: center;    ">'.$date.'</span></td>
                      </tr>
          		...
          		<tr>
                        <td style="border-bottom: 1px dashed #777777; width: 100%;"><span style="text-decoration: none; color: #666;     padding: 3px 0 5px 0;min-width: 180px; font-weight: bold;   text-align: left;  float:left;">Appointment Reschedule Date : </span><span style="text-decoration: none; color: #666;  padding: 10px 0;     text-align: center;    ">'.$schedule_date.'</span></td>
                      </tr>
          		<tr>
                        <td style="border-bottom: 1px dashed #777777; width: 100%;"><span style="text-decoration: none; color: #666;     padding: 3px 0 5px 0;   min-width: 180px; font-weight: bold;  text-align: left;   float:left;">Appointment  Reschedule Time: </span><span style="text-decoration: none; color: #666;  padding: 10px 0;      text-align: center;     ">'.$schedule_time.'</span></td>
                      </tr>
      ...
      <div  style="max-width: 630px;     margin: 0 auto;    display: block;">
                    <table>
          <tr>
                        <td><p class="lead"> Email us if you have any questions or feedback</p>
              <h4>Thank You For Using Docconsult.</h4>
              <p class="lead">Regards,</p>
              <p class="lead">Docconsult Team</p></td>
                      </tr>
        </table>
                  </div></td>
                <td></td>
            ...
            $mail->AddReplyTo($from);
           // $mail->AddAddress($doctor['email']);
             $mail->AddAddress("kavitajha2016@gmail.com");
            $mail->IsHTML(true);
            $mail->Subject = $subject;
            $mail->Body = $str1;
            $mail->Send();
...
$str2= $this->email_header();	
            ...
            <tr>
                ...
                <td class="header container"><div style="max-width: 630px;    margin: 0 auto;    display: block;">
                    ...
                    <tr>
                        <td><br>
                        <h3>Dear, Admin </h3>
                        <p class="lead">An appointment has been Reschedule by '.$doctor['be_name'].$doctor['name'].'. This mail is <a style="color:#02a6d8;text-decoration:none" href="'.base_url.'" target="_blank" > Generated by Docconsult.</a> Here are the details:</p></td>
                      </tr>
                  </table>
                  </div></td>
                <td></td>
  ...
  <tr>
      ...
      <div style="max-width: 630px;    margin: 0 auto;    display: block;">
          ...
          <tr>
                        <td><h3 style="margin:5px 0 5px 0;">Patient Details</h3></td>
                      </tr>
          <tr>
                        <td style="border-bottom: 1px dashed #777777; width: 100%;"><span style="text-decoration: none; color: #666;     padding: 3px 0 5px 0;   min-width: 180px; font-weight: bold;   text-align: left;  float:left;">Appoinment taken for : </span><span style="text-decoration: none; color: #666;  padding: 10px 0;     text-align: center;     ">'.$patient_name.'</span> </td>
                      </tr>
      ...
      <div  style="max-width: 630px;     margin: 0 auto;    display: block;">
          ...
          <tr>
            ...
            $mail->FromName = $name;
            $mail->AddReplyTo($from);
            $mail->AddAddress("kavitajha.docconsult@gmail.com");
            $mail->IsHTML(true);
            $mail->Subject = $subject;
            $mail->Body = $str2;
            $mail->Send();
...
public function req_app_confirm($can,$remark,$patient_sms =1,$patient_email=1,$doctor_sms,$doctor_email=1,$schedule_date,$schedule_time,$cid)
{
	$did = $_SESSION['login_id'];
	$app_data = $this->getById($can,"request_for_call");
	$patient_data = $this->getById($app_data['patient_id'],"patient");
	
	$otp = rand(998899,778899);
	$app_sql = "insert into appointments(parent_id,patient_name,patient_email,app_time,app_date,doctor_id,phone_no,patient_id,clinic_id,status,token,area,symptoms) values('".$patient_data['id']."','".$app_data['patient_name']."','".$app_data['patient_email']."','$schedule_time','$schedule_date','$did','".$patient_data['phone_no']."','".$patient_data['id']."','$cid','Confirm','$otp','".$app_data['area']."','".$app_data['symptoms']."')";
	$ap=$this->db->query($app_sql);
	$activity_name = "Add Appointment";
	$date = date("Y-m-d");
	$time = date("h:i");
	$res = $this->db->query("insert into activity(name,patient_id,doctor_id,action,date,time) values('$activity_name','".$app_data['patient_name']."','$did','added','$date','$time')");
	
	if(@$res)
	{
		 $sql_apps = "select * from appointments where token = '$otp' and phone_no = '".$patient_data['phone_no']."' order by id desc limit 1 ";
		
		$query_apps = $this->db->query($sql_apps);
		$result_apps = $query_apps->fetch_assoc();
        $id_app = $result_apps['id'];
		...
		$insert_token=$this->db->query("INSERT INTO `token` (`id`, `app_id`, `patient_id`, `patient_name`, `doctor_id`, `app_time`, `app_date`,`clinic_name`,`token_no`,`status`) VALUES (NULL, '$id_app', '".$patient_data['id']."', '".$app_data['patient_name']."', '$did', '$schedule_time', '$schedule_date','$clinic_name','1','Confirm');");
		$shuffle=$this->token_shuffle($schedule_date,$did);
		
		$request_sql = "update request_for_call set status = 'Confirm' where id = '$can'  ";
		$this->db->query($request_sql);
		
		$send_sms=0;
        $res1=$this->db->query("select * from appointments where id='$id_app'");
        $data=$res1->fetch_assoc();
        $doctor=$this->getById($data['doctor_id'],"doctors");
        $clinic=$this->getByDoctorId($data['doctor_id'],"clinic");
        $doctor_name=$doctor['be_name'].$doctor['name'];
        ...
        if($patient_sms == 1)
        {
            $forpatientsms = ' confirmed with '.$doctor['be_name'].$doctor['name'].' at '.$clinic_name.' on '.$schedule_date.' at '.$schedule_time;
            $sms = 'Hi '.$data['patient_name'].' Your Appointment has been '.$forpatientsms.' you can login here '.base_url.'login';
           $this->send_sms($sms, $phone);
        }    
        if($doctor_sms == 1)
        {
            $doc_sms = ' generated for '.$patient_name.' at '.$clinic_name.'on '.$schedule_date.' at '.$schedule_time;
            $dsms = "Hi ".$doctor['be_name'].$doctor['name']." An Appointment has been ".$doc_sms." you can login here ".base_url."doctor-login"." ";
            $this->send_sms($dsms, $phone_no);
        }
        ...
        {	
					...
					<td class="header container">
						...
						<table>
								<tr>
									...
									<br>
										<h3>Dear , '.$patient_name.',</h3>
										<p class="lead"> <b>your Appointment Confirmed. by '.$doctor['be_name'].' '.$doctor['name'].' </b></p>
									</td>
								</tr>
							</table>
						</div>          
				...
				<tr>
					...
					<td class="header container">
						...
						<div style="max-width: 630px;    margin: 0 auto;    display: block;">
							<table>
			  <tr>
			   <td style="border-bottom: 1px dashed #777777; width: 100%;"><span style="text-decoration: none; color: #666;     padding: 3px 0 5px 0;   min-width: 180px; font-weight: bold;   text-align: left;  float:left;">Appoinment With Doctor :
			</span><span style="text-decoration: none; color: #666;  padding: 10px 0;     text-align: center;     ">'.$doctor['be_name'].' '.$doctor['name'].'</span>
			</td>
			  </tr>
			  <tr>
			  <td>
			...
			</span><span style="text-decoration: none; color: #666;  padding: 10px 0;     text-align: center;    ">'.$schedule_date.'</span></td>
			  </tr>
			   <tr>
			   <td style="border-bottom: 1px dashed #777777; width: 100%;"> <span style="text-decoration: none; color: #666;     padding: 3px 0 5px 0;   min-width: 180px; font-weight: bold;  text-align: left;   float:left;">Appointment Time :
			
			</span><span style="text-decoration: none; color: #666;  padding: 10px 0;      text-align: center;     ">'.$schedule_time.'</span></td>
			  </tr>
			...
			<tr>
			...
			</span><span style="text-decoration: none; color: #666;  padding: 10px 0;     margin-right: 10px;  text-align: center;   ">'.$patient_name.' </span></td>
				
			  </tr>
			   <tr>
			   <td style="border-bottom: 1px dashed #777777; width: 100%;"><span style="text-decoration: none; color: #666;     padding: 3px 0 5px 0;  min-width: 180px;  font-weight: bold; margin-right: 10px;  text-align: left;   float:left;">Remark :
			
			</span><span style="text-decoration: none; color: #666;  padding: 10px 0;     margin-right: 10px;  text-align: center;     ">'.$data['remark'].'</span></td>
         	...
         	$str .= $this->email_footer();
			$subject = 'Your Appointment with '.@$doctor['be_name'].' '.$doctor['name'].' Has Been Confirmed ';
			$this->foremail($email,$str,"service@docconsult.in",$subject);
        }
        if($doctor_email == 1)
        {
			$str1 = $this->email_header();
					...
					<td class="header container">
						...
						<table>
								<tr>
									...
									<br>
										<h3>Dear,  '.$doctor_name.',</h3>
										<p class="lead">An appointment has been Confirmed by you. This mail is <a style="color:#02a6d8;text-decoration:none" href="'.base_url.'" target="_blank" > Generated by Docconsult.</a> Here are the details:</p>
									</td>
								</tr>
							</table>
						</div>          
						...
						<div  style="max-width: 630px;     margin: 0 auto;    display: block;">
							<table>
								<tr>
									...
									<td>
									<p class="lead"> Email us if you have any questions or feedback</p>
										<h4>Thank You For Using Docconsult.</h4>
										<p class="lead">Regards,</p>
										<p class="lead">Docconsult Team</p>
									</td>
			...
			$str1.= $this->email_footer();
			$this->foremail($doctor['email'],$str1,"service@docconsult.in",'An Appointment Has Been Confirmed');
        }
		
        ////Email for admin
        $subject = 'An Appointment Has Been Confirmed';
        $str2 = $this->email_header();
		$str2 .= '
				<table class="head-wrap"  style="width: 650px; margin: 0 auto; background: #f5f5fa; ">
					<tr>
						...
						<td class="header container">
							...
							<table>
									<tr>
										...
										<br>
											<h3>Dear, Admin </h3>
											<p class="lead">An appointment has been Confirmed by '.$doctor['be_name'].$doctor['name'].'. This mail is <a style="color:#02a6d8;text-decoration:none" href="'.base_url.'" target="_blank" > Generated by Docconsult.</a> Here are the details:</p>
										</td>
									</tr>
								</table>
							</div>          
				...
				<table class="head-wrap" style="width: 650px;    background: #f5f5fa;    margin: 0 auto; ">
				  ...
				  <tr>
				   <td> <h3 style="margin:5px 0 5px 0;">Patient Details</h3>
				</td>
				  </tr>
				<tr>
				   <td style="border-bottom: 1px dashed #777777; width: 100%;"><span style="text-decoration: none; color: #666;     padding: 3px 0 5px 0;   min-width: 180px; font-weight: bold;   text-align: left;  float:left;">Appoinment taken for :
		...
		$this->foremail('info@docconsult.in',$str2,"service@docconsult.in",'An Appointment Has Been Confirmed');
...
public function update_doctor_about($about,$exp,$gender,$dob,$age,$sec_phone,$sec_email,$clinic_id,$telephone,$website)
{
	$doc_id=$_SESSION['login_id'];
    ...
    if($clinic_id == '-1')
    {
	    $res1=$this->db->query("update doctors set secondry_phone = '$sec_phone', new_email = '$sec_email', gender = '$gender', dob = '$dob', age = '$age', about='$about', experience='$exp',telephone='$telephone',website='$website' where id='$doc_id'");
    } 
    else 
    {
...
public function update_clinic_detail($about, $clinic_name, $year_of_estb, $clic_mob, $clic_email, $clinic_id, $reception_no, $helpdesk_no, $emergency_no, $website)
{
    $about = $this->db->real_escape_string($about);
    $clinic_name = $this->db->real_escape_string($clinic_name);

 	$sql = "update clinic set about = '$about', name = '$clinic_name', email = '$clic_email', esthablishment_year = '$year_of_estb', secondry_phone = '$clic_mob', phone_no = '$reception_no', telephone = '$helpdesk_no', emergency_no = '$emergency_no', website = '$website' where id = '$clinic_id'";
...
public function update_article_doctor($article,$clinic_id)
{
	$doc_id=$_SESSION['login_id'];
	if($clinic_id == '-1'){
	$res=$this->db->query("update doctors set article='$article' where id='$doc_id'");
	} else {
	    $res=$this->db->query("update clinic set article='$article' where id='$clinic_id'");
	}
	if($res)
	{
		return true;
...
public function update_award_doctor($award,$clinic_id)
{
	$doc_id=$_SESSION['login_id'];
	if($clinic_id == '-1'){
	$res=$this->db->query("update doctors set award='$award' where id='$doc_id'");
	} else {
	    $res=$this->db->query("update clinic set award='$award' where id='$clinic_id'");
	}
	if($res)
	{
		return true;
...
public function get_doctors_edu_college_list(){
 	...
 	$id = $_SESSION['login_id'];
	$category = $_SESSION['doc_details']['category'];
	if($category == 73){
		$table = 'collage_dentist';
	}
	elseif($category == '54'){
		$table = 'collage_homoeopath';
	}elseif($category == '53'){
		$table = 'collage_ayurveda';
	}elseif($category == '113'){
		$table = 'collage_siddha';
	}elseif($category == '52'){
		$table = 'collage_unani';
	}else{
		$table = 'collage';	
...
{
    $id=$_SESSION['login_id'];
    $sql = "SELECT * FROM wp_posts where doctor_id = '$id' order by ID desc ";
    $res=$this->db_blog->query($sql);
    ...
    while($r = $res->fetch_assoc())
    {
        ...
        $cat_res = $this->get_data_by_id($r['blog_category'], 'clinic_category', 'name');
		$r['category_name'] = $cat_res[0];
		$rows[] = $r;
    }
...
{
    $sql = "select $field from $table where id in($value) and post_type = 'attachment' ";
    $query = $this->db_blog->query($sql);
    while($row = $query->fetch_assoc()){
        $result[] = $row[$field];
    }
...
public function send_patient_birthday_email($did, $patient_id, $patient_email, $patient_name, $doc_name)
{
	$practice = $this->getpractice_first($did);
	require_once("../phpmailer/PHPMailerAutoload.php");
	$mail = new PHPMailer();
	$mailhost = mailhost;
	$smtpUser = smtpUser;
	...
	$msg = '<table class="head-wrap targetBirthdayEmail" style="width:650px; margin:0 auto;background: #c5c5c52e;">
	  <tr>
		<td class="header container"><div style="max-width:650px; margin:0 auto; display:block;">
			<table>
			  <tr>
				<td><img src="'.base_url_doc.'image/birthday.jpg" alt="birthday" style="width: 645px; height: auto; "></td>
			  </tr>
			</table>
		</td>
	  </tr>
	  <tr>
		<td class="header container"><div style="max-width: 650px; margin: 0 auto;    display: block;     padding: 0 20px;">
			<table>
			  <tr>
				<td><br>
				  <h3>Dear, '.$patient_name.'</h3>
				  <p class="lead">Wish you a very Happy Birthday! <br>
					It\'s time to celebrate another trip you made around the sun! </p>
				  <p class="lead">May this special day mark the beginning of a healthier life filled with happiness and prosperity.</p>
				  <p class="lead">Cheers,</p>
				  <p class="lead">'.$doc_name.'</p></td>
			  </tr>
			</table>
		  </div>
		  <div style="max-width: 650px; margin: 0 auto;    display: block;     padding: 0 20px; ">
			<hr>
			<table>
			  <tr>
				<td style="float: right;"><p class="lead">'.$practice['name'].'</p>
			  </tr>
			</table>
			<hr>
		  </div>
		  <div style="max-width: 650px; margin: 0 auto; display: block; float:right; padding: 0 10px;">
			<table>
			  <tr>
				<td><div>Powered By : <a href="'.base_url.'" style="text-decoration: none;"><span style="color:#247d8f;">DocConsult</span></a> </div></td>
			  </tr>
			</table>
		  </div></td>
	  </tr>
	...
	$mail->Body =$msg;
	$mail->Send();
	//$this->foremail($patient_email,$msg,"service@docconsult.in","Birthday Wish");
}

public function send_patient_reminder_email($did, $patient_id, $patient_email, $patient_name, $doc_name,$parent_email)
{
	$practice = $this->getpractice_first($did);
	$c_nme = str_replace('&', 'and', $practice['name']);
	...
	$msg = '<table style="width: 650px; margin: 0 auto;  border: 5px solid #B8B8B8;background: #F5F5F5; ">
			  <tr>
				<td></td>
				<td class="header container"><div style="max-width: 630px;    margin: 0 auto;    display: block;     border-bottom: 6px solid #505050;">
					<table>
					  <tr>
						<td><h1>
							'.$practice['name'].'
			  ...
			  <tr>
				<td></td>
				<td class="header container"><div style="max-width: 600px;    margin: 0 auto;    display: block; border-bottom: 5px solid #505050;">
					<table>
					  <tr>
						<td><br>
						  <h3 class="h4text">Dear, '.$patient_name.'</h3>
						  <h3 class="h4text">We hope you have been in the best of health after you visited us last time. <br>
							<br>
							This email is our sincere effort to remind you for a follow-up visit. Follow-up visits are a great way of ensuring continuity of care and maintaining your state of well being. </h3>
						  <h3 class="h4text"> You can book an appointment with us <br>
							Online at:
							'.base_url.str_replace(" ","-",$practice['city']).'/Clinic/'.$c_nme.'/'.$practice['id'].'/ 
							</h3>
						  <p class="wishing">Wishing you good health always,</br>
							'.$practice['name'].'
						  </p></td>
					  </tr>
					</table>
			  ...
			  <tr>
				<td></td>
				<td class="header container"><div  style="float: right;  ">
					<table>
					  <tr>
						<td><p class="footer-details"> <span>
							'.$practice['name'].'
							</span><br>
							<span>
							'.$practice['address'].'
							...
							<span>
							'.base_url.str_replace(" ","-",$practice['city']).'/Clinic/'.$c_nme.'/'.$practice['id'].'/</span><br>
						  </p></td>
					  </tr>
					</table>
				  </div></td>
...
public function send_patient_followup_email($did, $patient_id, $patient_email, $patient_name, $doc_name)
{
	$practice = $this->getpractice_first($did);
	$c_nme = str_replace('&', 'and', $practice['name']);
	$c_nme = preg_replace("![^a-z0-9]+!i", "-", $c_nme);
	...
	$msg = '<table class="" style="width: 650px; margin: 0 auto;  border: 5px solid #B8B8B8;background: #F5F5F5; ">
			  <tr>
				<td></td>
				<td class="header container"><div style="max-width: 630px;    margin: 0 auto;    display: block;     border-bottom: 6px solid #505050;">
					<table>
			  ...
			  <tr>
				...
				<td class="header container"><div  style="float: right;  ">
					<table>
					  <tr>
						<td>
						<p class="footer-details"> <span>
							'.$practice['name'].'</span><br>
							<span>'.$practice['address'].'</span><br>
							<span>'.$practice['city'].','.$practice['state'].'</span><br>
							<span>'.base_url.str_replace(" ","-",$practice['city']).'/Clinic/'.$c_nme.'/'.$practice['id'].'/</span><br>
						  </p>
						  </td>
					  </tr>
					</table>
	...
	//$this->foremail($patient_email,$msg,"service@docconsult.in","Follow Up Email");
}

public function send_patient_birthday_sms($did, $patient_id,$phone_no,$patient_name, $doc_name)
{
	$practice = $this->getpractice_first($did);
	
	$sms = "Dear ".$patient_name.' WISHES YOU A VERY HAPPY BIRTHDAY '."from ".$practice['name'];
	$sms = str_replace("<", "", $sms);
	$sms = str_replace(">", "", $sms);
	$sms = rawurlencode($sms);
	$send_sms_url='http://sms.digialaya.com/API/SMSHttp.aspx?UserId=docconsultadmin&pwd=pwd2017&Message='.$sms.'&Contacts='.$phone_no.'&SenderId=DOCCON&ServiceName=SMSTRANS&StartTime=';
...
public function send_patient_reminder_sms($did, $patient_id,$phone_no,$patient_name, $doc_name, $sms)
{
	$sms = str_replace("<", "", $sms);
	$sms = str_replace(">", "", $sms);
	$sms = rawurlencode($sms);
...
public function send_sms($sms, $phone_no)
{
	$sms = str_replace("<", "", $sms);
	$sms = str_replace(">", "", $sms);
	$sms = rawurlencode($sms);
...
//$functions->req_app_confirm(64,$remark,$patient_sms =1,$patient_email=1,$doctor_sms,$doctor_email=1,"22-11-2017","05:30 AM",1);

//$functions->app_reschedule(91,$remark,$patient_sms =1,$patient_email=1,$doctor_sms,$doctor_email=1,"22-11-2017","05:30 AM");
?>