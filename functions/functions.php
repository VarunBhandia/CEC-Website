...
public function searchcity($city)
    {
        $res=$this->db->query("select DISTINCT city from clinic where city like '%$city%' order by city_count desc, city limit 5");
...
public function getarea($city, $district, $limit)
{
    $result = $this->db->query("SELECT distinct area FROM clinic WHERE city = '$city' AND area like '%$district%' order by location_count desc, area limit $limit");
        while($row=mysqli_fetch_assoc($result)) {
            $resultset[] = $row;
        }       
...
public function get_doc_cat1($dis,$area,$spec){
    $arr = array();
    $doclen =strlen($spec);
	$search_query = $search_query1 = $search_query2 = $search_query3 = '';
    if(!empty($area))
    {
		$search_query = " and doctors.location='$area' ";
		$search_query1 = " and t2.location='$area' ";
		$search_query2 = " and area='$area' ";
		$search_query3 = " and location='$area' ";
	}
	if ($doclen>=3)
	{
		$res2 = $this->db->query("select doctors.id as id ,doctors.category as category, doctors.name as name,doctors.image as image,doctors.location as location, doctors.type as doc_type, doctors.be_name as bename from doctors inner join practice on doctors.id=practice.doctor_id and doctors.city = '$dis' $search_query and doctors.status != 'Suspended' where name like '%$spec%' group by doctors.id limit 5");        
		if (strpos('DOCTORS', strtoupper($spec)) !== false) 
		{
	...
	/*$res = $this->db->query("SELECT t1.category as cat FROM `category` as t1 inner join doctors as t2 on t1.id = t2.category WHERE t2.city='$dis' $search_query1 and t1.category like '%$spec%' group by cat limit 5");
	if($res->num_rows > 0)
	{
		$arr1['type']='';
		$arr1['category']="Speciality";
		$arr1['image']='';
		$arr1['disabled']= true;
		$arr[] = $arr1;
		while($data = $res->fetch_assoc()){
			$arr1['type']="Speciality";
			$arr1['category']=$data['cat'];
			$arr1['image']='';
			$arr1['disabled']= false;
			$arr[] = $arr1;
		}
	...
	if ($doclen>=3)
	{
		$res_clinic = $this->db->query("select image, id, name from clinic where city='$dis' $search_query2 and name like '%$spec%' limit 5");
		if($res_clinic->num_rows > 0){   
			$arr4['type']="";
			$arr4['image']='';
			$arr4['index']='';
			$arr4['category']='Clinics';
			$arr4['disabled']= false;
			$arr[]=$arr4;
			while($data = $res_clinic->fetch_assoc()){
				$arr4['type']="Clinic";
		...
		$sql = "select services from doctors where city = '$dis' $search_query3 and services != '' ";
		...
		if($query->num_rows > 0) { 
			...
			if(count($values) > 0){
				...
				$service_sql = "select * from services_for_seo_use where id in ($values) and services like '%$spec%' order by services limit 5";
				$service_query = $this->db->query($service_sql);
				if($service_query->num_rows > 0){
					$service_result['type']='';
					$service_result['category'] = "Service";
					$service_result['image']='';
					$service_result['disabled']=true;
					$arr[] = $service_result;
					while($rows = $service_query->fetch_assoc()){
						$service_result['type']='Service';
						$service_result['category'] = $rows['services'];
						$service_result['image']='';
						$service_result['disabled']= false;
						$arr[] = $service_result;
					}
				}
			}
	...
	$sql_spec = "select specialization from doctors where city = '$dis' $search_query3 and specialization != '' ";
	...
	if($query_s->num_rows > 0){
		...
		if(count($values1) > 0){
			...
			$doc_sepec_sql = "select distinct specialization from speciality where id in ($values1) and specialization like '%$spec%' order by specialization limit 10";
			$doc_sepec_query = $this->db->query($doc_sepec_sql);
			if($doc_sepec_query->num_rows > 0){
				$doc_sepec_result['type']='';
				$doc_sepec_result['category'] = "Speciality";
				$doc_sepec_result['image']='';
				$doc_sepec_result['disabled']=true;
				$arr[] = $doc_sepec_result;
				while($rows = $doc_sepec_query->fetch_assoc())
				{
					$doc_sepec_result['type']='Speciality';
					$doc_sepec_result['category'] = $rows['specialization'];
					$doc_sepec_result['image']='';
					$doc_sepec_result['disabled']= false;
					$arr[] = $doc_sepec_result;
				}
			}
		}
    ...
    if ($doclen>=3)
	{
		$clinic_spec_data = $this->db->query("select t1.category from clinic_category as t1 inner join clinic as t2 on t1.id = t2.category WHERE t2.city='$dis' $search_query1 and t1.category like '%$spec%' group by t1.category");
			$clinic_spec['type']='';
            $clinic_spec['category'] = "Clinic Speciality";
            $clinic_spec['image']='';
            $clinic_spec['disabled']=true;
			$arr[]=$clinic_spec;
		if($clinic_spec_data->num_rows > 0){
			while($data = $clinic_spec_data->fetch_assoc()){
				$clinic_spec['type']="Clinic Speciality";
                $clinic_spec['category'] = $data['category'];
                $clinic_spec['image']='';
				$clinic_spec['disabled']= false;
				$arr[] = $clinic_spec;
			}
		}
	}
...
public function get_doc_treatments_list($dis,$area,$type,$limit)
{
...
public function get_doc_treatments_list_by_specialiaztion($dis,$area,$type)
{
    ...
    //$sql = "select services from doctors where city = '$dis' and type = 'Doctor' and category = '$type' and services != '' $search order by rand() $lmt";
	$sql = "select services from doctors where city = '$dis' and type = 'Doctor' and services != '' $search  group by category order by rand() $lmt";
    $res = $this->db->query($sql);
    ...
    $keys = array_unique($ids_val);
    return $keys;
}
    
public function get_category_index(){
    $res = $this->db->query("SELECT * FROM doctor_tip1 INNER JOIN `condition` ON doctor_tip1.Category = `condition`.id");
    $result = array();
    while($data = $res->fetch_assoc())
    {
        $result[] = $data['name'];
...
public function get_vaccine_index(){ 
    $res = $this->db->query("SELECT * FROM newvaccination");
    $result = array();
    while($data = $res->fetch_assoc())
    {
        $result[] = $data['vaccine'];
...
public function get_vaccine_id(){ 
    $res = $this->db->query("SELECT * FROM newvaccination");
    $result = array();
    while($data = $res->fetch_assoc())
    {
...
public function get_dose_index(){ 
    $res = $this->db->query("SELECT * FROM newvaccination");
    $result = array();
    while($data = $res->fetch_assoc())
    {
...
public function get_prevent_index(){ 
    $res = $this->db->query("SELECT * FROM newvaccination");
    $result = array();
    while($data = $res->fetch_assoc())
    {
        $result[] = $data['prevents'];
    }
    return $result;
}
public function get_rem1_index(){ 
    $res = $this->db->query("SELECT * FROM patient_vaccine_record");
    $result = array();
    while($data = $res->fetch_assoc())
    {
        $result[] = $data['rem1'];
...
public function get_rem2_index(){ 
    $res = $this->db->query("SELECT * FROM patient_vaccine_record");
    $result = array();
    while($data = $res->fetch_assoc())
    {
...
public function get_rem3_index(){ 
    $res = $this->db->query("SELECT * FROM patient_vaccine_record");
    $result = array();
    while($data = $res->fetch_assoc())
    {
...
public function insert_patient_vaccine($patient_id){
    $sqlpat = "SELECT * FROM `patient` WHERE id = $patient_id";
    $resultpat = $functions->db->query($sqlpat);
    $rowpat = $resultpat->fetch_assoc();
    $dateOfBirth = $rowpat['dob'];
    $diff = date_diff(date_create($dateOfBirth), date_create($date1));
    $dose1 = $diff->format('%a');
    $all_vaccine_id = $functions->get_vaccine_id();
    $all_vaccine = $functions->get_vaccine_index();
    $all_prevents = $functions->get_prevent_index();
    $all_doses = $functions->get_dose_index();
    $cnt_vaccine = count($all_vaccine);
    $all_dose1 = $functions->get_dose_index();
    for($i = 0; $i <$cnt_vaccine ; $i++)
    {
        $dose1 = $all_dose1[$i];
        $vaccine_date = date('d-m-Y',strtotime( $dateOfBirth." + ".$dose1."days"));
        $sql = "INSERT INTO `patient_vaccine_record`(`patient_id`, `vaccine_id`, `name`, `dose1`, `date1`, `prevents`,) VALUES ('$patient_id','$all_vaccine_id[$i]','$all_vaccine[$i]','$all_dose1[$i]','$vaccine_date','$all_prevents[$i]')";
    }
    
}
public function auto_mail_remainder(){
    $rem1 = $functions->get_rem1_index();
    $rem2 = $functions->get_rem2_index();
    $rem3 = $functions->get_rem3_index();
    $rem1_cnt = count($rem1);
    $today = date("Y-m-d");
    for($i = 0; $i <$rem1_cnt ; $i++)
    {
        echo $rem1[$i];
        if($rem1[$i] == $today){
            $mail = $functions->foremail('varunbhandia@gmail.com','msg','service@docconsult.in','subjct');
            if($mail)
            {
                print 'ok mail sent';    
            }else{
...
public function get_takenon_user($id){ 
    $res = $this->db->query("SELECT * FROM patient_vaccine_record where patient_id = '$id' ");
    $result = array();
    while($data = $res->fetch_assoc())
    {
        $result[] = $data['takenon'];
...
public function get_vaccine_index_user($id){ 
    $res = $this->db->query("SELECT * FROM patient_vaccine_record where patient_id = '$id' ");
    $result = array();
    while($data = $res->fetch_assoc())
    {
...
public function get_dose_index_user($id){ 
    $res = $this->db->query("SELECT * FROM patient_vaccine_record where patient_id = '$id' ");
    $result = array();
    while($data = $res->fetch_assoc())
    {
...
public function get_vaccine_id_user($id){ 
    $res = $this->db->query("SELECT * FROM patient_vaccine_record where patient_id = '$id' ");
    $result = array();
    while($data = $res->fetch_assoc())
    {
...
public function get_prevent_index_user($id){ 
    $res = $this->db->query("SELECT * FROM patient_vaccine_record where patient_id = '$id'");
    $result = array();
    while($data = $res->fetch_assoc())
    {
...
public function get_takenon_user_mail(){ 
    $res = $this->db->query("SELECT * FROM patient_vaccine_record");
    $result = array();
    while($data = $res->fetch_assoc())
    {
...
public function get_vaccine_index_user_mail(){ 
    $res = $this->db->query("SELECT * FROM patient_vaccine_record");
    $result = array();
    while($data = $res->fetch_assoc())
    {
...
public function get_dose_index_user_mail(){ 
    $res = $this->db->query("SELECT * FROM patient_vaccine_record");
    $result = array();
    while($data = $res->fetch_assoc())
    {
...
public function get_vaccine_id_user_mail(){ 
    $res = $this->db->query("SELECT * FROM patient_vaccine_record");
    $result = array();
    while($data = $res->fetch_assoc())
    {
...
public function get_prevent_index_user_mail(){ 
    $res = $this->db->query("SELECT * FROM patient_vaccine_record");
    $result = array();
    while($data = $res->fetch_assoc())
    {
...
public function get_related_posts_index($cat_id){
    $res = $this->db->query("SELECT * FROM doctor_tip1 WHERE Category = '$cat_id'");
    $result = array();
    while($data = $res->fetch_assoc())
    {
        $result[] = $data['Topic'];
...
public function search_by_service_counts($city,$loc,$serv,$page,$gender,$experience,$price,$day, $locale) {
    ...
    if ($gender != 'all'){
        $genderque="and t1.gender='$gender'";
    } else {
        $genderque="";
    }
    if ($locale != '' and $locale != 'all'){
        ...
        $localeque=" and t1.location in('$locale')";
    } else {
        $localeque="";
    }
    if ($price != 'all'){
        ...
        $price1=min($price);
        $price2=max($price);
        $priceque=" and t2.consult between '$price1' and '$price2' ";
    } else {
        $priceque="";
    }
    if ($experience != 'all'){
        $curr_year = date("Y");
        $experiencefrom = $curr_year - $experience;
        $experienceto = $experiencefrom + 5;
        
        $expque = " and experience between '$experiencefrom' and '$experienceto' ";   
    } else {
        $expque = "";
    }
    if($day != 'all' AND $day !=''){ 
        ...
        switch($filter_day){
            ...
            case sat:
                ...
                break;
            case tod:
                $fil_day = strtolower(date('l'));
                break;    
        }
        $dayque="and t2.$fil_day != '' ";
    } else if ($day =='all'){
        $dayque="";
    } else {
        $fil_day = strtolower(date('l'));
    ...
    if(!empty($loc) && $locale == ''){
        $res1 = $this->db->query("select t1.id,t1.services, t2.sunday, t2.monday, t2.tuesday, t2.wednesday, t2.thursday, t2.friday, t2.saturday from doctors as t1 inner join practice as t2 on t1.id = t2.doctor_id where t1.city='$city' and t1.location='$loc' $genderque $dayque $expque $priceque and t1.services!='' group by t2.doctor_id ");
    }
    else {
        $res1 = $this->db->query("select t1.id,t1.services, t2.sunday, t2.monday, t2.tuesday, t2.wednesday, t2.thursday, t2.friday, t2.saturday from doctors as t1 inner join practice as t2 on t1.id = t2.doctor_id  where t1.city='$city' $genderque $dayque $expque $priceque $localeque and t1.services!=''  group by t2.doctor_id ");
        //print "select t1.id,t1.services from doctors as t1 inner join practice as t2 on t1.id = t2.doctor_id  where t1.city='$city' $genderque $dayque $expque and t1.services!=''  group by t2.doctor_id ";
    }
...
public function search_by_service($city,$loc,$serv,$page,$gender,$experience,$price,$day, $locale) {
    ...
    if(!empty($loc) && $locale == ''){
        $res1 = $this->db->query("select t1.*, t2.sunday, t2.monday, t2.tuesday, t2.wednesday, t2.thursday, t2.friday, t2.saturday from doctors as t1 inner join practice as t2 on t1.id = t2.doctor_id where t1.city='$city' and t1.location='$loc' $genderque $dayque $expque $priceque and t1.services!='' group by t2.doctor_id ");
    }
    else {
        print "select t1.*, t2.sunday, t2.monday, t2.tuesday, t2.wednesday, t2.thursday, t2.friday, t2.saturday from doctors as t1 inner join practice as t2 on t1.id = t2.doctor_id  where t1.city='$city' $genderque $dayque $expque $priceque $localeque and t1.services!='' group by t2.doctor_id ";
        $res1 = $this->db->query("select t1.*, t2.sunday, t2.monday, t2.tuesday, t2.wednesday, t2.thursday, t2.friday, t2.saturday from doctors as t1 inner join practice as t2 on t1.id = t2.doctor_id  where t1.city='$city' $genderque $dayque $expque $priceque $localeque and t1.services!='' group by t2.doctor_id ");
    }
...
public function search_by_specialization_counts($city,$loc,$serv,$page,$gender,$experience,$price,$day, $locale) 
{
    $doc_id=array();
    ...
    if(!empty($loc) && $locale == ''){
        $res1 = $this->db->query("select t1.*, t2.sunday, t2.monday, t2.tuesday, t2.wednesday, t2.thursday, t2.friday, t2.saturday from doctors as t1 inner join practice as t2 on t1.id = t2.doctor_id where t1.city='$city' and t1.location='$loc' $genderque $dayque $expque $priceque and t1.specialization !='' group by t2.doctor_id ");
    }
    else {
        $res1 = $this->db->query("select t1.*, t2.sunday, t2.monday, t2.tuesday, t2.wednesday, t2.thursday, t2.friday, t2.saturday from doctors as t1 inner join practice as t2 on t1.id = t2.doctor_id where t1.city='$city' $genderque $dayque $expque $priceque $localeque and t1.specialization !='' group by t2.doctor_id ");
    }
    ...
    if($res1->num_rows > 0){
        ...
        while($data = $res1->fetch_assoc()){
            ...
            foreach ($services as $key => $value) 
            {
				...
				elseif(trim($serv) == $value['specialization'])
                {   
                    $doc_id[] = $data;
                }
            }
...
public function search_by_specialization($city,$loc,$serv,$page,$gender,$experience,$price,$day, $locale) {
...
public function doctor_submit_chat_box_consult($doctor_mobile,$doctor_name,$answer,$id)
{
    $sql="UPDATE chat_consult SET doctor_name='$doctor_name', doctor_mobile='$doctor_mobile', answer='$answer', doctor_timestamp=CURRENT_TIMESTAMP WHERE id='$id';";

           $this->db->query($sql);
    return 1;
}

public function question_chat_box_consult($mobile,$name,$category,$question,$email,$pub,$gender,$datebirth,$area,$height,$weight,$askedone,$askedtwo,$askedthree,$symptoms) {

    $sql="INSERT INTO chat_consult (category, name, email, mobile,question,public, patient_timestamp, gender, datebirth, area, height, weight, askedone, askedtwo, askedthree, symptoms) VALUES ('$category', '$name','$email', '$mobile','$question', '$pub', CURRENT_TIMESTAMP,'$gender','$datebirth','$area','$height','$weight','$askedone','$askedtwo','$askedthree','$symptoms');";
...
public function ask_outside_question_chat_box_consult($mobile,$name,$category,$question,$pub,$gender,$datebirth,$area,$height,$weight,$askedone,$askedtwo,$askedthree,$symptoms){
    $sql="INSERT INTO chat_consult (mobile,category, name,question,public, patient_timestamp, gender, datebirth, area, height, weight, askedone, askedtwo, askedthree, symptoms) VALUES ('$mobile','$category', '$name','$question', '$pub', CURRENT_TIMESTAMP,'$gender','$datebirth','$area','$height','$weight','$askedone','$askedtwo','$askedthree','$symptoms');"; 

           $this->db->query($sql);        
        return 1;
}

public function patient_details_chat_box_consult($mobile){
    $sql="SELECT * FROM chat_consult WHERE mobile='$mobile';" ; 
      $result = $this->db->query("$sql");
      while($row=mysqli_fetch_assoc($result)) {
          $resultset[] = $row;
      }       
